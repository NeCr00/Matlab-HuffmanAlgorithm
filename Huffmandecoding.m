function deco = Huffmandecoding( sig, dict  )
    ...
    while( ~isempty(sig_) ) % For each set of bits in the signal.
        ...
        while (1) % Loop
            ...
            if ( length(dictb.codewords) ~= 1 ) % Until one codeword left.
                ...
                sig_ = sig_(length(dictb.codewords{1})+1:end); % Update the input signal.
                break;
            end        
        end
    ...
    function [flag,dict_] = found_match( code, pos, dict )
        dict_.symbol={}; dict_.codewords={}; % Create a dictionary structure.
        j = 1;    % Iterator.
        flag = 0; % Flag for error.
        for i = 1:length(dict.codewords) % For each code in dictionary.
            if ( strcmp(dict.codewords{i}(pos), code) ) % If inpute code matches 
                 flag = 1; % No error match.
                 dict_.symbol(j) = dict.symbol(i); % Get the symbol that matches.
                 dict_.codewords(j) = dict.codewords(i);     % Get the code of the matched symbol.
